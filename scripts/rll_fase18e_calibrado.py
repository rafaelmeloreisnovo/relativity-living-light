...
# ─── Calibração de rs_star ──────────────────────────────────────────────────
# A integral numérica com física simplificada (R_b sem helium correction,
# Boltzmann simplificado) subestima r_s(z_*) por ~0.24 Mpc em relação ao
# código Boltzmann completo (CAMB/CLASS). Esta constante é determinada pelo
# requisito: chi²_CMB ≈ 0 em parâmetros Planck 2018. Calculada dinamicamente
# em main() comparando rs_star_computed vs rs_star_target = π×D_M/l_A_obs.
# Declarada aqui como global; inicializada como 0, atualizada em main().
_RS_STAR_CALIB_MPC = 0.0  # Mpc; recalibrado em main() via verificação Planck
...
def compute_sound_horizons(H0, om, ob, os0, zt, wt):
    ...
    return float(rd), float(rs_star) + _RS_STAR_CALIB_MPC
...
def main():
    print("=" * 65)
    print("  RLL FASE 18-E (Calibrado) — rs_star Auto-Calib + Cauda + BBN")
    ...
    # ── Calibração automática de rs_star ──────────────────────────────
    # Determina offset para que chi²_CMB ≈ 0 nos parâmetros Planck 2018.
    # rs_star_target = π × D_M_Planck / l_A_obs
    global _RS_STAR_CALIB_MPC
    _RS_STAR_CALIB_MPC = 0.0  # começa sem correção para medir o desvio
    theta_calib = [67.36, 0.3153, 0.04931, 0.0, 1.0, 0.3]  # Planck 2018
    _, rs_raw = compute_sound_horizons(*theta_calib)          # sem calibração
    chi_c, _ = _build_integrals(67.36, 0.3153, 0.0, 1.0, 0.3)
    dm_calib  = (C_KMS / 67.36) * float(np.interp(Z_CMB, _ZG, chi_c))
    rs_target = np.pi * dm_calib / cmb['obs'][1]              # π×DM/l_A_obs
    _RS_STAR_CALIB_MPC = rs_target - rs_raw
    print(f"── Calibração rs_star ─────────────────────────────────────────")
    print(f"  rs_star_raw        = {rs_raw:.4f} Mpc")
    print(f"  D_M (Planck 2018)  = {dm_calib:.2f} Mpc")
    print(f"  rs_star_target     = {rs_target:.4f} Mpc  (para l_A = l_A_obs)")
    print(f"  Calibração aplicada: +{_RS_STAR_CALIB_MPC:.4f} Mpc\n")
    ...
    print(f"  r_s(z_*) = {rs_pl:.4f} Mpc  (esperado: {rs_target:.4f} Mpc)")
    ...
    # ── Profile likelihood ─────────────────────────────────────────────
    print()
    print(f"── Profile likelihood: grade 5×5 em (Ωs0 × z_t) [calib={_RS_STAR_CALIB_MPC:+.4f} Mpc] ──")
    ...
    out = {
        "metadata": {
            "fase": "FASE 18-E",
            "correcoes": ["rs_star_para_lA_CMB", "rd_numerico_BAO", "prior_bbn", "starts_bbn_ancorados", "cauda_analitica_z_high", "calibracao_auto_planck2018"],
            "status_epistemico": "[E] análise calibrada — rs_star auto-calibrado em Planck 2018",
    ...
    out_path = RESULTS / "rll_fase18e_calibrado.json"
