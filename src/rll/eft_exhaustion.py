...
class RLLParameters:
    ...
    def omega_lambda_closure(self) -> float:
        ...
        return 1.0 - float(self.omega_m) - float(self.omega_r) - float(self.alpha) * float(
            s_transition(1.0, self.k, self.a_t)
        )
...
def canonical_scalar_reconstruction(a: np.ndarray, params: RLLParameters) -> dict[str, object]:
    ...
    return {
        ...
        "reason": "canonical V(phi) reconstruction computed"
        if valid
...
def identifiability_scan(alpha_values: Iterable[float], k_values: Iterable[float], at_values: Iterable[float]) -> dict[str, object]:
    grid = [(alpha, k, at) for alpha in alpha_values for k in k_values for at in at_values]
    non_identifiable = any(k == 0.0 for _, k, _ in grid) or any(alpha == 0.0 for alpha, _, _ in grid)
    return {
        "grid_points": len(grid),
        "non_identifiable": bool(non_identifiable),
        "reason": f"{TOKEN_VAZIO}: parâmetros não identificáveis em alpha=0 ou k=0"
        if non_identifiable
...
def load_observational_evidence(inputs: ArtifactInputs) -> dict[str, object]:
    ...
    comparisons: dict[str, object] = {"RLL_vs_LCDM": TOKEN_VAZIO, "RLL_vs_w0waCDM": TOKEN_VAZIO}
    rll_row = rows.get("RLL")
    for baseline_name, report_key in (("LCDM", "RLL_vs_LCDM"), ("w0waCDM", "RLL_vs_w0waCDM"), ("w0wa", "RLL_vs_w0waCDM")):
        ...
        rll_chi2 = _float_from_row(rll_row, "chi2_total") or _float_from_row(rll_row, "chi2")
        base_chi2 = _float_from_row(baseline_row, "chi2_total") or _float_from_row(baseline_row, "chi2")
        if rll_aic is None or base_aic is None:
            continue
        comparisons[report_key] = {
            "baseline": baseline_name,
            "delta_aic_rll_minus_baseline": rll_aic - base_aic,
            "delta_bic_rll_minus_baseline": None if rll_bic is None or base_bic is None else rll_bic - base_bic,
            "delta_chi2_rll_minus_baseline": None if rll_chi2 is None or base_chi2 is None else rll_chi2 - base_chi2,
            "decision_by_delta_aic": _aic_decision(rll_aic - base_aic),
        }

    missing_required = [name for name in DATASETS_REQUIRED if dataset_status.get(name) == TOKEN_VAZIO]
    missing_baselines = [name for name in BASELINES_REQUIRED if comparisons.get(f"RLL_vs_{name}") == TOKEN_VAZIO]
    return {
        ...
        "source_artifacts": {
            "validation_summary": str(inputs.validation_summary) if inputs.validation_summary else None,
            "model_comparison_csv": str(inputs.model_comparison_csv) if inputs.model_comparison_csv else None,
        },
    }


def final_decision(stability: Mapping[str, object], evidence: Mapping[str, object], identifiability: Mapping[str, object]) -> str:
    ...
    if not isinstance(lcdm, Mapping) or not isinstance(w0wa, Mapping):
        return "INCONCLUSIVO (TOKEN_VAZIO)"
    if lcdm.get("decision_by_delta_aic") == "favorável" and w0wa.get("decision_by_delta_aic") == "favorável":
        return "ACEITO COMO TEORIA FUNDAMENTAL"
    if lcdm.get("decision_by_delta_aic") == "rejeitado" or w0wa.get("decision_by_delta_aic") == "rejeitado":
        return "REJEITADO COMO EFT"
    return "INCONCLUSIVO (TOKEN_VAZIO)"
...
def write_markdown_report(report: Mapping[str, object], path: Path) -> None:
    ...
    lines.extend(["", "## Comparação estatística", "| comparação | resultado |", "| --- | --- |"])
    if isinstance(comparison, Mapping):
        for key, value in comparison.items():
            lines.append(f"| {key} | {json.dumps(value, ensure_ascii=False)} |")
    lines.extend(["", "## Observação", "Nenhuma conclusão é promovida quando derivação, dado ou baseline obrigatório está ausente.", ""])
...
def run_exhaustion(
    ...
    report = {
        ...
        "artifacts": {"reconstruction_csv": str(csv_path), "json_report": str(report_path), "markdown_report": str(markdown_path)},
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--validation-summary", type=Path, default=Path("validacao_real/results/validation_summary.json"))
    parser.add_argument("--model-comparison-csv", type=Path, default=Path("validacao_real/results/model_comparison.csv"))
    args = parser.parse_args(argv)
    report = run_exhaustion(
        RLLParameters(args.omega_m, args.omega_r, args.omega_lambda, args.alpha, args.k, args.a_t),
        args.out_dir,
        args.grid,
