...
6. O checkout não persiste credenciais.
7. O workflow possui apenas `contents: read`.
8. Não existe commit automático de resultados.
9. Comandos são executados por vetores `argv`, sem `eval`, `os.popen` ou `shell=True`.
10. O modo de pull request é sempre `dry_run`.
...
O workflow antigo, com 1.149 linhas, foi reduzido a um controlador que:

1. faz checkout seguro;
2. configura Python 3.11;
3. instala somente o bootstrap do gate;
4. executa `tools/rll_pipeline_deterministic.py`;
5. publica logs, relatórios, contratos e checksums com `if: always()`.
...
O workflow leve `python-tests.yml` permanece disponível como CI automático geral. A FASE 24.1 não cria novos workflows paralelos.
...
Este documento é o commit final de organização e rastreabilidade da série.

## 7. Estado de validação

Validações estáticas realizadas antes do PR:

- sintaxe Python dos novos arquivos analisada;
- sintaxe YAML do controlador analisada;
- branch comparada com `main`: `ahead`, sem commits atrás;
- sete arquivos alterados antes deste registro;
- nenhum dado científico modificado.

Validação em runner permanece pendente até a execução observável do pull request. Até haver run, logs e artifact:
...
O rollback é a reversão integral do PR. Como os dados científicos não foram alterados e não existe escrita automática, não há migração de dados ou recuperação de estado externo.

---

`ψ intenção → χ observação → ρ falso-verde → Δ gate determinístico → Σ evidência → Ω claim bloqueado até prova`
