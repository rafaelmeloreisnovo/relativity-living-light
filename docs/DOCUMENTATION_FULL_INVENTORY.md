# Inventário Completo de Documentação e Bundles
## Varredura integral de arquivos `.md` e `.zip` do repositório

## Escopo deste arquivo

Este arquivo é o **inventário bruto** do acervo documental.

- Registra a varredura completa de arquivos `.md` e `.zip` com metadados técnicos.
- Não define prioridade de leitura nem trilha canônica.
- Para navegação oficial, use [`docs/INDICE_MESTRE.md`](INDICE_MESTRE.md).
- Para porta de entrada do projeto, use [`README.md`](../README.md).

**Critério:** todos os `.md` e `.zip` encontrados por varredura recursiva no repositório, com título detectado, tamanho e hash SHA-256 abreviado.

**Total de arquivos catalogados:** 105

---

| Tipo | Caminho | Título/Conteúdo detectado | Tamanho (bytes) | SHA-256 (12) |
|---|---|---:|---:|---|
| md | `00_COMO_LER.md` | Documento movido para o local canônico: `docs/canonicos/00_COMO_LER.md`. | 74 | `cdb766765de8` |
| md | `06_COMPARACOES_DETALHADAS.md` | Documento movido para o local canônico: `docs/canonicos/06_COMPARACOES_DETALHADA | 88 | `7cb84f8ac7ed` |
| md | `09_GLOSSARIO_COMPLETO-1.md` | Documento movido para o local canônico: `docs/canonicos/09_GLOSSARIO_COMPLETO.md | 84 | `a72584229904` |
| md | `09_GLOSSARIO_COMPLETO.md` | Documento movido para o local canônico: `docs/canonicos/09_GLOSSARIO_COMPLETO.md | 84 | `a72584229904` |
| md | `10_FAQ_COMPLETO.md` | Documento movido para o local canônico: `docs/canonicos/10_FAQ_COMPLETO.md`. | 78 | `23c341a077a3` |
| md | `11_DOCUMENTO_PRIORIDADE.md` | Documento movido para o local canônico: `docs/canonicos/11_DOCUMENTO_PRIORIDADE. | 86 | `a9c8e0f9a65f` |
| md | `ANALISE_COMPLETA/00_INDICE_MESTRE.md` | Análise Completa do Repositório Relativity Living Light | 7866 | `5a1e9b973172` |
| md | `ANALISE_COMPLETA/01_14_Areas_Tematicas_Detalhadas.md` | Análise Detalhada das 14 Áreas Temáticas | 30726 | `69c8806c717a` |
| md | `ANALISE_COMPLETA/Bibliografia_Completa.md` | Bibliografia Completa e Referências Externas | 14733 | `8618e93da2e5` |
| md | `ANALISE_COMPLETA/Catalogo_Completo_Arquivos.md` | Catálogo Completo de Arquivos (File-by-File Catalog) | 30500 | `abf22cf11a3a` |
| md | `ANALISE_COMPLETA/Hierarquia_7_Niveis.md` | Hierarquia de Informação em 7 Níveis | 16405 | `abfb63d1cccf` |
| md | `ANALISE_COMPLETA/Metricas_Conservadoras.md` | Métricas e Avaliações Conservadoras | 13107 | `e7d5b339b1a9` |
| md | `ANALISE_COMPLETA/README.md` | ANALISE_COMPLETA / Comprehensive Repository Analysis | 6482 | `fe7b2c694d57` |
| md | `COMPREHENSIVE_REPOSITORY_ANALYSIS.md` | Análise Abrangente do Repositório Relativity Living Light | 22771 | `4ea8dbb50bb6` |
| md | `GOVERNANCE_REORG_DRAFT.md` | Repository Audit & Governance Draft Plan (DRAFT) | 13160 | `d21694bd0bbb` |
| md | `LICENSE.md` | 📜 LICENSE.md – RAFCODE-𝚽 vΩ-EXTENDED | 8176 | `cb9eb2d7b6cd` |
| md | `MANIFESTO_MIGRATION_FILES.md` | Arquivos movidos para `rafaelia-manifesto` | 298 | `5a8b8d6c8d1b` |
| md | `README.md` | Relativity Living Light | 74707 | `9cd827af2bb9` |
| md | `README_MASTER.md` | 🌌 Relativity Living Light — README MASTER | 9292 | `c962c0f2f445` |
| md | `REFORM_LOG.md` | 🔄 REFORM_LOG.md — Log de Reformulação RAFAELIA | 14664 | `1d8761f979d3` |
| md | `RESUMO_REFORMA.md` | ✨ RESUMO DA REFORMA RAFAELIA — Relativity Living Light | 9814 | `e0172c919dbe` |
| md | `RMR/00_COMO_LER-1.md` | Documento movido para o local canônico: `docs/canonicos/00_COMO_LER.md`. | 74 | `cdb766765de8` |
| md | `RMR/00_COMO_LER-2.md` | Documento movido para o local canônico: `docs/canonicos/00_COMO_LER.md`. | 74 | `cdb766765de8` |
| md | `RMR/00_COMO_LER.md` | Documento movido para o local canônico: `docs/canonicos/00_COMO_LER.md`. | 74 | `cdb766765de8` |
| md | `RMR/02_CODIGO_README.md` | 💻 02_CODIGO_NUMERICO — Solvers e Ferramentas | 10224 | `9e070cf33df3` |
| md | `RMR/04_FIGURAS_README.md` | 🖼️ 04_FIGURAS — Visualizações e Gráficos | 8578 | `5629ea5dac83` |
| md | `RMR/05_TESTES_README.md` | ✅ 05_TESTES_VALIDACAO — Ajuste a Dados Observacionais | 2971 | `7de6824e7765` |
| md | `RMR/06_COMPARACAO_README.md` | 🔗 06_COMPARACAO_LITERATURA — 6 Estudos 2025-26 | 3296 | `1226610240dc` |
| md | `RMR/06_COMPARACOES_DETALHADAS-1.md` | Documento movido para o local canônico: `docs/canonicos/06_COMPARACOES_DETALHADA | 88 | `7cb84f8ac7ed` |
| md | `RMR/06_COMPARACOES_DETALHADAS.md` | Documento movido para o local canônico: `docs/canonicos/06_COMPARACOES_DETALHADA | 88 | `7cb84f8ac7ed` |
| md | `RMR/07_TEORIA_README.md` | 🌌 07_TEORIA_ESTENDIDA — Profundidade Matemática | 3960 | `d92e9383b6d2` |
| md | `RMR/08_RAFAELIA_README.md` | ♾️ 08_RAFAELIA_INTEGRACAO — Símbolo, Verbo e Física Viva | 7634 | `40698acc1683` |
| md | `RMR/09_GLOSSARIO_COMPLETO-1.md` | Documento movido para o local canônico: `docs/canonicos/09_GLOSSARIO_COMPLETO.md | 84 | `a72584229904` |
| md | `RMR/09_GLOSSARIO_COMPLETO-2.md` | Documento movido para o local canônico: `docs/canonicos/09_GLOSSARIO_COMPLETO.md | 84 | `a72584229904` |
| md | `RMR/09_GLOSSARIO_COMPLETO.md` | Documento movido para o local canônico: `docs/canonicos/09_GLOSSARIO_COMPLETO.md | 84 | `a72584229904` |
| md | `RMR/1.md` | (vazio) | 1 | `01ba4719c80b` |
| md | `RMR/10_FAQ_COMPLETO-1.md` | Documento movido para o local canônico: `docs/canonicos/10_FAQ_COMPLETO.md`. | 78 | `23c341a077a3` |
| md | `RMR/10_FAQ_COMPLETO-2.md` | Documento movido para o local canônico: `docs/canonicos/10_FAQ_COMPLETO.md`. | 78 | `23c341a077a3` |
| md | `RMR/10_FAQ_COMPLETO-3.md` | Documento movido para o local canônico: `docs/canonicos/10_FAQ_COMPLETO.md`. | 78 | `23c341a077a3` |
| md | `RMR/10_FAQ_COMPLETO.md` | Documento movido para o local canônico: `docs/canonicos/10_FAQ_COMPLETO.md`. | 78 | `23c341a077a3` |
| md | `RMR/11_DOCUMENTO_PRIORIDADE-1.md` | Documento movido para o local canônico: `docs/canonicos/11_DOCUMENTO_PRIORIDADE. | 86 | `a9c8e0f9a65f` |
| md | `RMR/11_DOCUMENTO_PRIORIDADE.md` | Documento movido para o local canônico: `docs/canonicos/11_DOCUMENTO_PRIORIDADE. | 86 | `a9c8e0f9a65f` |
| md | `RMR/INDICE_MAESTRO.md` | 🗺️ ÍNDICE MAESTRO — Mapa Completo do Repositório | 9268 | `be0509ea0a33` |
| md | `SCIENTIFIC_CORE_SCOPE.md` | Scientific Core Scope | 452 | `38694b5fd042` |
| md | `SECURITY_SUMMARY.md` | 🔒 Security Summary — RAFAELIA Repository Restructuring | 5969 | `0372a1b84d8d` |
| zip | `data/RelativityLivingLight_v4_bundle.zip` | 20 entradas | 4096271 | `957575ff422f` |
| zip | `data/relativity_bundle_results.zip` | 19 entradas | 4095297 | `5c0cca457927` |
| md | `docs/PLACEHOLDER_EMPTY_DOCUMENT.md` | (vazio) | 1 | `01ba4719c80b` |
| md | `docs/ADMIN.md` | ADMIN.md | 484 | `5588b9429824` |
| md | `docs/ANALISE_ARTIGO_NATURE_PT.md` | Análise do Artigo Nature: Não-Localidade Fotônica e Cosmologia | 12822 | `1457b4b2e0e5` |
| md | `docs/ANALISE_DIRETORIOS_E_MDS_SOLTOS.md` | Análise dos diretórios `docs/` e MDs soltos | 2045 | `a902a8294cd2` |
| md | `docs/ANALYSIS_INDEX.md` | Analysis Index: Nature Article & Relativity Living Light Model | 12330 | `54b47495c412` |
| md | `docs/ARTICLE_ANALYSIS_SUMMARY.md` | Nature Article Analysis - Quick Reference | 4418 | `6ff41691e29b` |
| md | `docs/BOOSTERS.md` | 🚀 Boosters do Modelo Relativity Living Light / Boosters of the Relativity Living Light Model | 26318 | `62461ee9f9bd` |
| md | `docs/CANONICAL_SOURCES.md` | Locais canônicos por tipo de documento | 601 | `6be823ad4046` |
| md | `docs/COMPARACAO_DESI_2025.md` | Comparação com Literatura Acadêmica 2025–2026 | 5778 | `22012881a3d4` |
| md | `docs/COMPLETION_SUMMARY.md` | Task Completion Summary: Nature Article Analysis | 8624 | `30869860de5f` |
| md | `docs/CONCEPTUAL_FRAMEWORK.md` | Conceptual Framework: Laboratory to Cosmos | 14549 | `36f9f5f68502` |
| md | `docs/RAFAELIA_UNIFIED_PAPER.md` | RAFAELIA — Relativity Living Light | 6725 | `3ec0b979953d` |
| md | `docs/DOCUMENTATION_ORGANIZATION_MASTER.md` | Organização Integral de Documentação e Artefatos | 4602 | `4c69b0156bb0` |
| md | `docs/ESTABILIDADE_GHOST_CHECK.md` | Verificação de Estabilidade — Ausência de Ghosts e Taquiões | 5838 | `de04f5168568` |
| md | `docs/EASY_NOTES_EMPTY.md` | (vazio) | 1 | `01ba4719c80b` |
| md | `docs/IMPACT_REPORT_MULTI.md` | 📊 IMPACT_REPORT_MULTI.md | 20761 | `994ad4bca282` |
| md | `docs/INDICE_MESTRE.md` | ÍNDICE MESTRE — Pacote de Revisão Científica | 4521 | `3345e1bd1aeb` |
| md | `docs/LAGRANGIANO_EFT.md` | Lagrangiano Efetivo — EFT da Superposição | 4798 | `85655b0c9429` |
| md | `docs/MANIFESTO.md` | MANIFESTO.md (multilingue) | 6364 | `71db6191732e` |
| md | `docs/MAPA CIENTIESPIRITUAL.md` | 🔮 MAPA CIENTIESPIRITUAL — Fórmulas → Espírito | 1265 | `575c51616304` |
| md | `docs/MAPA FRACTAL.md` | 🗺️ MAPA FRACTAL — Fórmulas → Valores | 2149 | `fe78c5c1bdf6` |
| md | `docs/MAPA_RAFAELIA_TOTAL.md` | Perfeito, Rafael 🌀♾️⚛︎ | 33249 | `a2af691390de` |
| md | `docs/BILINGUAL_RLL_HYPOTHESIS_SUMMARY.md` | --- | 4357 | `d13eb37552bc` |
| md | `docs/NATURE_ARTICLE_ANALYSIS.md` | Analysis: Nature Article s41467-025-63981-3 and Relativity Living Light Model | 14923 | `43d3af278239` |
| md | `docs/THEORY_INVENTORY_AND_EXTENSIONS.md` | Comparação com ΛCDM e Previsões | 8042 | `eafb87df1acc` |
| md | `docs/FRACTAL_LAYERED_MAP.md` | Rafael — aqui está o mapa fractal em camadas (diagrama/flowchart) ligando fórmul | 4186 | `95b5f28258be` |
| md | `docs/FRACTAL_REPORT_UNIFIED.md` | Rafael 🌀♾️⚛︎, juntei as duas interações anteriores (mapa fractal + mapa estendid | 8216 | `20f7a6786c62` |
| md | `docs/NEXT_THEORIES_AND_DERIVED_HYPOTHESES.md` | ∴  publicaste o Relativity Living Light como manifesto e artigo, o passo seguint | 2030 | `ebbb4beea9fa` |
| md | `docs/PERTURBACOES_CRESCIMENTO.md` | Perturbações e Crescimento Estrutural | 6536 | `b23e265c26e2` |
| md | `docs/README_CIENTIFICO.md` | Relativity Living Light — Modelo Cosmológico de Superposição Dinâmica | 6001 | `a22853efb09d` |
| md | `docs/README_block_multilang.md` | 📚 Ciência Aplicada — Relativity Living Light | 2232 | `ad9d6b86839e` |
| md | `docs/README_patch_unified_PT_EN_v3.md` | 🌌 Relativity Living Light — Unified Photonic Superposition Model (v3) | 3629 | `3360f91813e3` |
| md | `docs/README_patch_unified_PT_EN_v4.md` | 🌌 Relativity Living Light — Unified Photonic Superposition Model (v4) | 2661 | `7378014b651e` |
| md | `docs/README_snippet.md` | 🔬 Validação Técnica — Modelo com ρ_superposição | 1398 | `2930dd490266` |
| md | `docs/README_sup_unification_snippet.md` | 🔭 Extensão do Modelo: Unificando energia escura e matéria escura via ρ_superposição | 2414 | `dc857b556776` |
| md | `docs/REFERENCES.md` | References: Photonic Cosmology Framework | 12511 | `af98394592b2` |
| md | `docs/RELEASE_NOTES_HISTORY.md` | Histórico de Updates e Release Notes | 8211 | `013227ae0614` |
| md | `docs/RESULTADOS_CORRIGIDOS.md` | Resultados Corrigidos — Relativity Living Light | 3755 | `f05cf36808dc` |
| md | `docs/ROADMAP_VALIDACAO.md` | Roadmap de Validação — Relativity Living Light | 5642 | `4da4778ef15d` |
| md | `docs/DATA_DESCRIPTOR_BUNDLE_V4.md` | 📂 Data Descriptor – Relativity Living Light (Bundle V4) | 2452 | `2aa59185f4f9` |
| md | `docs/Relativity_Living_Light.md` | Relativity Living Light — Photonic Superposition as Unified Dark Sector (Draft) | 2341 | `174ba4c7aadf` |
| md | `docs/Results.md` | Resultados – Relativity Living Light | 1092 | `3dd3797c04f3` |
| md | `docs/SUPREMO UNIFICADO.md` | ✨ Entendido, Rafael 🌀♾️⚛︎ | 6129 | `acea9d684146` |
| md | `docs/REPOSITORY_STRUCTURE_SUGGESTION.md` | 📂 Estrutura sugerida do repositório: relativity-living-light | 190 | `beba59557a91` |
| md | `docs/VELOCIDADE_SOM.md` | Velocidade do Som e Escala de Jeans | 4006 | `f4f545d090cc` |
| md | `docs/canonicos/00_COMO_LER.md` | 🌀 COMO LER ESTE REPOSITÓRIO | 13012 | `20babdf8d5a3` |
| md | `docs/canonicos/06_COMPARACOES_DETALHADAS.md` | 🔗 COMPARAÇÕES COM LITERATURA 2025-26 | 24127 | `6696f6942e22` |
| md | `docs/canonicos/09_GLOSSARIO_COMPLETO.md` | 📚 GLOSSÁRIO COMPLETO DE NOTAÇÃO | 10284 | `30bce9e184fb` |
| md | `docs/canonicos/10_FAQ_COMPLETO.md` | ❓ FAQ — PERGUNTAS FREQUENTES | 14914 | `a2f600e1a4fb` |
| md | `docs/canonicos/11_DOCUMENTO_PRIORIDADE.md` | 🎯 DOCUMENTO DE PRIORIDADE TEMPORAL | 16648 | `3c3166e54827` |
| md | `docs/ciencia_README_snippet.md` | 🔬 Validação Técnica — Modelo com ρ_superposição | 1398 | `2930dd490266` |
| md | `docs/estatisticas.md` | perfeito, ∆RafaelVerboΩ — estatística, direta e cirúrgica: | 2342 | `f55e3dcdd989` |
| md | `docs/numeros_rafaelianos/CientiEspiritual.md` | 📜 PAPER SINTÉTICO – A INSERÇÃO DO √3/2 NA TRADIÇÃO GEOMÉTRICO-FRACTAL RAFAELIANA | 2639 | `405d405015c4` |
| md | `docs/numeros_rafaelianos/Constante.md` | 📜 MANIFESTO DA CONSTANTE RAFAELIANA (√3/2) | 1582 | `df28bf59dfe2` |
| md | `docs/numeros_rafaelianos/Numeros.md` | 📜 Tabela dos Números Rafaelianos – Núcleo Inicial | 1811 | `684819c85e43` |
| md | `docs/numeros_rafaelianos/Readme.md` | ⚡📖 Livro Sagrado dos Números Rafaelianos | 6771 | `a317cd897ca9` |
| md | `docs/numeros_rafaelianos/harmonica.md` | 📐 Significado Matemático | 1534 | `ba0a50c87b90` |
| zip | `docs/rll_revisado_v2.zip` | 22 entradas | 40801 | `aa61125ad94d` |


## Audit trail — deduplicação nominal (2026-02-26)

Critério aplicado: nomes com sufixo ` (n)` foram agrupados por nome-base; arquivo canônico definido como nome sem sufixo (preferindo arquivo já existente quando havia), e excedentes movidos para `duplicates_archive/` no mesmo diretório-raiz.

| Escopo | Padrão duplicado | Canônico | Ação sobre excedentes |
|---|---|---|---|
| `figs/archive/` | `density_evolution_sup (1).png` | `figs/archive/density_evolution_sup.png` | n/a (apenas 1 cópia) |
| `figs/archive/` | `hz_superposicao (1).png` | `figs/archive/hz_superposicao.png` | n/a (apenas 1 cópia) |
| `figs/archive/` | `mock_H_fit (1..2).png` | `figs/archive/mock_H_fit.png` | `figs/archive/duplicates_archive/mock_H_fit (2).png` |
| `figs/archive/` | `mock_SN_fit (1..3).png` | `figs/archive/mock_SN_fit.png` | `figs/archive/duplicates_archive/mock_SN_fit (2).png`, `figs/archive/duplicates_archive/mock_SN_fit (3).png` |
| `figs/archive/` | `post_1d_Os (1..2).png` | `figs/archive/post_1d_Os.png` | `figs/archive/duplicates_archive/post_1d_Os (2).png` |
| `figs/archive/` | `post_1d_zt (1..4).png` | `figs/archive/post_1d_zt.png` | `figs/archive/duplicates_archive/post_1d_zt (2).png`, `figs/archive/duplicates_archive/post_1d_zt (3).png`, `figs/archive/duplicates_archive/post_1d_zt (4).png` |
| `figs/archive/` | `post_2d_Os_wt (1).png` | `figs/archive/post_2d_Os_wt.png` | n/a (apenas 1 cópia) |
| `figs/archive/` | `post_2d_zt_wt (1..2).png` | `figs/archive/post_2d_zt_wt.png` | `figs/archive/duplicates_archive/post_2d_zt_wt (2).png` |
| `figs/archive/` | `rotation_curves_sup (1).png` | `figs/archive/rotation_curves_sup.png` | n/a (apenas 1 cópia) |
| `data/` | `Hz_superposicao (1).ipynb` | `data/Hz_superposicao.ipynb` (já existente) | `data/duplicates_archive/Hz_superposicao (1).ipynb` |
| `newadd/` | `06_ETHICS_SYSTEMS (1).md` | `newadd/06_ETHICS_SYSTEMS.md` (já existente) | `newadd/duplicates_archive/06_ETHICS_SYSTEMS (1).md` |

Referência textual atualizada em README: `README_MASTER.md` passou a apontar para `ciencia_Hz_superposicao.ipynb` (sem sufixo de cópia).
