# Real Numbers Report

Gerado em: `2026-06-13T11:45:35.740849+00:00`  
Commit: `69b48e1c653383122cd148ca17e92363510f9333`

| Métrica | Valor |
|---|---:|
| `tracked_files_total` | 943 |
| `cataloged_files` | 925 |
| `uncataloged_or_error_files` | 18 |
| `total_bytes` | 35239380 |
| `total_text_lines` | 140940 |
| `markdown_files` | 471 |
| `yml_yaml_files` | 61 |
| `github_workflow_yml_files` | 23 |
| `data_or_result_files` | 111 |

## Categorias

| Categoria | Arquivos |
|---|---:|
| `documentation_md` | 471 |
| `source_code` | 146 |
| `other` | 129 |
| `data_or_result` | 111 |
| `config_yml` | 38 |
| `github_workflow_yml` | 23 |
| `archive` | 6 |
| `test_or_validation` | 1 |

## Extensões

| Extensão | Arquivos |
|---|---:|
| `.md` | 471 |
| `.py` | 134 |
| `.png` | 67 |
| `.csv` | 65 |
| `.yml` | 61 |
| `.json` | 59 |
| `NO_EXT` | 12 |
| `.txt` | 10 |
| `.sh` | 6 |
| `.zip` | 6 |
| `.ipynb` | 5 |
| `.s` | 4 |
| `.c` | 4 |
| `.tsv` | 4 |
| `.java` | 3 |
| `.tex` | 3 |
| `.h` | 2 |
| `.pdf` | 2 |
| `.sha256` | 2 |
| `.cff` | 1 |
| `.html` | 1 |
| `.svg` | 1 |
| `.toml` | 1 |
| `.ini` | 1 |

## Flags

| Flag | Ocorrências |
|---|---:|
| `cosmology_validation_marker` | 414 |
| `token_vazio_declared` | 99 |
| `result_artifact` | 91 |
| `mentions_mock_or_placeholder` | 85 |
| `mentions_synthetic` | 67 |

## Fronteira

tracked_files_total é a contagem bruta de `git ls-files`. cataloged_files é o que foi lido, medido e hasheado. uncataloged_or_error_files mostra a lacuna real.
