# Real Numbers Report

Gerado em: `2026-06-04T13:52:13.392597+00:00`  
Commit: `044952e6a19de0a750815ef7d36840f03643218d`

| Métrica | Valor |
|---|---:|
| `tracked_files_total` | 714 |
| `cataloged_files` | 696 |
| `uncataloged_or_error_files` | 18 |
| `total_bytes` | 33867099 |
| `total_text_lines` | 114758 |
| `markdown_files` | 357 |
| `yml_yaml_files` | 29 |
| `github_workflow_yml_files` | 13 |
| `data_or_result_files` | 69 |

## Categorias

| Categoria | Arquivos |
|---|---:|
| `documentation_md` | 357 |
| `other` | 120 |
| `source_code` | 114 |
| `data_or_result` | 69 |
| `config_yml` | 16 |
| `github_workflow_yml` | 13 |
| `archive` | 6 |
| `test_or_validation` | 1 |

## Extensões

| Extensão | Arquivos |
|---|---:|
| `.md` | 357 |
| `.py` | 102 |
| `.png` | 63 |
| `.csv` | 55 |
| `.json` | 30 |
| `.yml` | 29 |
| `NO_EXT` | 12 |
| `.txt` | 8 |
| `.sh` | 6 |
| `.zip` | 6 |
| `.ipynb` | 5 |
| `.s` | 4 |
| `.c` | 4 |
| `.java` | 3 |
| `.tex` | 3 |
| `.h` | 2 |
| `.pdf` | 2 |
| `.cff` | 1 |
| `.tsv` | 1 |
| `.toml` | 1 |
| `.ini` | 1 |
| `.sha256` | 1 |

## Flags

| Flag | Ocorrências |
|---|---:|
| `cosmology_validation_marker` | 325 |
| `mentions_mock_or_placeholder` | 63 |
| `result_artifact` | 57 |
| `mentions_synthetic` | 43 |
| `token_vazio_declared` | 7 |

## Fronteira

tracked_files_total é a contagem bruta de `git ls-files`. cataloged_files é o que foi lido, medido e hasheado. uncataloged_or_error_files mostra a lacuna real.
