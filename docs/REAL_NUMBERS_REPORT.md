# Real Numbers Report

Gerado em: `2026-06-13T08:13:11.219618+00:00`  
Commit: `cd2e7d9778286c574a65b7ac9382baceb329aaab`

| Métrica | Valor |
|---|---:|
| `tracked_files_total` | 857 |
| `cataloged_files` | 839 |
| `uncataloged_or_error_files` | 18 |
| `total_bytes` | 34844654 |
| `total_text_lines` | 130695 |
| `markdown_files` | 428 |
| `yml_yaml_files` | 49 |
| `github_workflow_yml_files` | 17 |
| `data_or_result_files` | 94 |

## Categorias

| Categoria | Arquivos |
|---|---:|
| `documentation_md` | 428 |
| `source_code` | 132 |
| `other` | 129 |
| `data_or_result` | 94 |
| `config_yml` | 32 |
| `github_workflow_yml` | 17 |
| `archive` | 6 |
| `test_or_validation` | 1 |

## Extensões

| Extensão | Arquivos |
|---|---:|
| `.md` | 428 |
| `.py` | 120 |
| `.png` | 67 |
| `.csv` | 62 |
| `.yml` | 49 |
| `.json` | 47 |
| `NO_EXT` | 12 |
| `.txt` | 10 |
| `.sh` | 6 |
| `.zip` | 6 |
| `.ipynb` | 5 |
| `.s` | 4 |
| `.c` | 4 |
| `.java` | 3 |
| `.tex` | 3 |
| `.h` | 2 |
| `.tsv` | 2 |
| `.pdf` | 2 |
| `.sha256` | 2 |
| `.cff` | 1 |
| `.html` | 1 |
| `.svg` | 1 |
| `.toml` | 1 |
| `.ini` | 1 |

## Flags

| Flag | Ocorrências |
|---|---:|
| `cosmology_validation_marker` | 390 |
| `mentions_mock_or_placeholder` | 81 |
| `result_artifact` | 78 |
| `mentions_synthetic` | 63 |
| `token_vazio_declared` | 62 |

## Fronteira

tracked_files_total é a contagem bruta de `git ls-files`. cataloged_files é o que foi lido, medido e hasheado. uncataloged_or_error_files mostra a lacuna real.
