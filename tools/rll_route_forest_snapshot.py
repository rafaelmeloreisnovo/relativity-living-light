#!/usr/bin/env python3
"""Materialize an effective RLL route-forest blueprint from base + overlay.

The base blueprint remains historical. The overlay records the exact snapshot
migration, route patches, route additions and path normalizations required for
one coherent event window. This module never promotes scientific claims.
"""
...
def apply_overlay(base: dict[str, Any], overlay: dict[str, Any]) -> dict[str, Any]:
    ...
    result = _normalize_refs(result, replacements)
...
def main() -> int:
    ...
    try:
        effective = load_effective_blueprint(args.base, args.overlay)
    ...
    print(
        json.dumps(
            {
                "base_forest_id": args.overlay.name,
